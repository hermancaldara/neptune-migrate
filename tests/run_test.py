...
"""
    ...
    @patch("sys.stdout", new_callable=StringIO)
    def test_it_should_print_neptune_migrate_version_and_exit(self, stdout_mock):
        try:
            run.run_from_argv(["-v"])
    ...
    def test_it_should_print_message_and_exit_when_user_interrupt_execution(
        self, stdout_mock, show_colors_mock
    ):
    ...
    @patch("neptune_migrate.cli.CLI.show_colors")
    @patch("sys.stdout", new_callable=StringIO)
    def test_it_should_print_message_and_exit_when_an_error_happen(
        self, stdout_mock, show_colors_mock
    ):
    ...
    def test_it_should_read_configuration_file_using_fileconfig_class_and_execute_with_default_configuration(
        self, get_variables_from_file_mock, main_mock, execute_mock
    ):
        ...
        self.assertEqual(
            os.path.abspath("."), config_used.get("database_migrations_dir")
        )
    ...
    def test_it_should_get_configuration_exclusively_from_args_if_not_use_configuration_file_using_config_class_and_execute_with_default_configuration(
        ...
        self.assertEqual(1, main_mock.call_count)
        config_used = main_mock.call_args[0][0]
        self.assertIsInstance(config_used, neptune_migrate.config.Config)
        ...
        self.assertEqual("ontology", config_used.get("database_ontology"))
        self.assertEqual(
            os.path.abspath("../migration"), config_used.get("database_migrations_dir")
        )
        self.assertEqual(False, config_used.get("show_sparql"))
        self.assertEqual(False, config_used.get("show_sparql_only"))
    ...
    def test_it_should_use_log_level_as_specified(
    ...
    @patch("neptune_migrate.run.getpass", return_value="password_asked")
    @patch("sys.stdout", new_callable=StringIO)
    @patch.object(neptune_migrate.main.Main, "execute")
    @patch.object(neptune_migrate.main.Main, "__init__", return_value=None)
    @patch.object(
    ...
    def test_it_should_ask_for_password_when_configuration_is_as_ask_me(
        self, import_file_mock, main_mock, execute_mock, stdout_mock, getpass_mock
    ):
        run.run_from_argv(["-c", os.path.abspath("sample.conf")])
        config_used = main_mock.call_args[0][0]
    ...
    def test_it_should_use_password_from_command_line_when_configuration_is_as_ask_me(
        ...
        run.run_from_argv(
            ["-c", os.path.abspath("sample.conf"), "--db-password", "xpto_pass"]
        )
        config_used = main_mock.call_args[0][0]
