...
from neptune_migrate.main import Main
...
class MainTest(BaseTest):
    ...
    def test_it_should_raise_error_if_a_required_config_to_migrate_is_missing(self):
        self.assertRaisesWithMessage(
            Exception, "invalid key ('database_host')", Main, Config()
        )
        self.assertRaisesWithMessage(
            Exception,
        ...
        self.assertRaisesWithMessage(
            ...
            Main,
            Config({"database_host": "", "database_endpoint": ""}),
        )
        self.assertRaisesWithMessage(
            Exception,
            "invalid key ('database_password')",
            Main,
            Config({"database_host": "", "database_endpoint": "", "database_user": ""}),
        )
    ...
    @patch("neptune_migrate.main.SimpleVirtuosoMigrate")
    @patch("neptune_migrate.main.LOG")
    @patch("neptune_migrate.main.CLI")
    def test_it_should_use_the_other_utilities_classes(
        self, cli_mock, log_mock, simplevirtuosomigrate_mock
    ):
    ...
    def test_it_should_raise_error_if_config_is_not_an_instance_of_neptune_migrate_config(
        self,
    ):
        self.assertRaisesWithMessage(
            Exception,
            "config must be an instance of neptune_migrate.config.Config",
            Main,
            config={},
        )

    @patch("neptune_migrate.main.Main._execution_log")
    @patch("neptune_migrate.main.Main._run_after")
    @patch("neptune_migrate.main.Main._load_triples", return_value=None)
    ...
    @patch("neptune_migrate.main.Main._execution_log")
    @patch("neptune_migrate.main.Main._load_triples", return_value=None)
    @patch("neptune_migrate.main.Main._valid_version")
    def test_should_exec_and_call_run_after_script(
        self, valid_version_mock, load_triples_mock, execution_log_mock
    ):
        self.initial_config.update(
            {"load_ttl": "", "run_after": "tests/samples/validate_run_after.py"}
        )
        main = Main(Config(self.initial_config))
        main.execute()
    ...
    @patch("neptune_migrate.main.Main._execution_log")
    @patch("neptune_migrate.main.Main._load_triples")
    def test_should_exec_and_fail_with_invalid_after_script(
        self, load_triples_mock, execution_log_mock
    ):
        self.initial_config.update(
            {"load_ttl": "", "run_after": "tests/samples/invalid_run_after.py"}
        )
        main = Main(Config(self.initial_config))
        main.execute()
    ...
    @patch("neptune_migrate.main.Main._execution_log")
    @patch("neptune_migrate.main.Virtuoso.execute_change")
    @patch(
        "neptune_migrate.main.Virtuoso.get_current_version", return_value=(None, None)
    )
    @patch("neptune_migrate.main.Virtuoso._run_isql", return_value=("", ""))
    def test_it_should_add_triples_if_the_database_is_empty_and_the_option_is_activated_by_the_user(
        self,
        run_isql_mock,
        current_version_mock,
        execute_change_mock,
        _execution_log_mock,
    ):
        self.initial_config.update(
            {"load_ttl": "new_triple.ttl", "show_sparql_only": None}
        )
        main = Main(Config(self.initial_config))
        main.execute()
    ...
    @patch("neptune_migrate.main.Virtuoso.execute_change")
    @patch(
        "neptune_migrate.main.Virtuoso.get_current_version", return_value=("0.1", "git")
    )
    @patch("neptune_migrate.main.Virtuoso._run_isql", return_value=("", ""))
    ...
    def test_it_should_not_add_triples_if_show_sparql_only_option_is_activated_by_the_user(
        self, current_version_mock, execute_change_mock, _execution_log_mock
    ):
        self.initial_config.update(
            {"load_ttl": "new_triple.ttl", "show_sparql_only": True}
        )
        main = Main(Config(self.initial_config))
        main.execute()
    ...
    @patch("neptune_migrate.main.Main._execution_log")
    @patch("neptune_migrate.main.Main._migrate")
    def test_it_should_migrate_db_if_create_migration_option_is_not_activated_by_user(
        self, migrate_mock, _execution_log_mock
    ):
        config = Config(self.initial_config)
        main = Main(config)
        main.execute()
    ...
    @patch("neptune_migrate.main.SimpleVirtuosoMigrate")
    @patch("neptune_migrate.main.LOG.debug")
    @patch("neptune_migrate.main.CLI")
    def test_it_should_write_the_message_to_log(
        self, cli_mock, log_mock, simplevirtuosomigrate_mock
    ):
    ...
    @patch("neptune_migrate.main.SimpleVirtuosoMigrate")
    @patch("neptune_migrate.main.LOG")
    @patch("neptune_migrate.main.CLI.msg")
    def test_it_should_write_the_message_to_cli(
        self, cli_mock, log_mock, simplevirtuosomigrate_mock
    ):
        main = Main(config=Config(self.initial_config))
    ...
    @patch("neptune_migrate.main.LOG")
    @patch("neptune_migrate.main.CLI.msg")
    def test_it_should_write_the_message_to_cli_using_default_color(
        self, cli_mock, log_mock, simplevirtuosomigrate_mock
    ):
    ...
    @patch("neptune_migrate.main.Main._execute_migrations")
    @patch(
        "neptune_migrate.main.Main._get_destination_version",
        return_value="destination_version",
    )
    @patch(
        "neptune_migrate.main.Virtuoso",
        return_value=Mock(
            **{
                "get_current_version.return_value": ("current_version", "git"),
                "get_sparql.return_value": ("sparql_up", "sparql_down"),
            }
        ),
    )
    def test_it_should_get_current_and_destination_versions_and_execute_migrations(
        self, virtuoso_mock, _get_destination_version_mock, execute_migrations_mock
    ):
        main = Main(Config(self.initial_config))
        main.execute()
        execute_migrations_mock.assert_called_with(
            "sparql_up", "sparql_down", "current_version", "destination_version"
        )

    @patch(
        "neptune_migrate.main.SimpleVirtuosoMigrate",
        return_value=Mock(**{"check_if_version_exists.return_value": True}),
    )
    def test_it_should_get_destination_version_when_user_informs_a_specific_version(
        self, simplevirtuosomigrate_mock
    ):
        self.initial_config.update({"schema_version": "20090214115300"})
        main = Main(Config(self.initial_config))
        self.assertEqual("20090214115300", main._get_destination_version())
        main.virtuoso_migrate.check_if_version_exists.assert_called_with(
            "20090214115300"
        )
    ...
    @patch(
        "neptune_migrate.main.SimpleVirtuosoMigrate",
        return_value=Mock(
            **{"latest_version_available.return_value": "latest_version_available"}
        ),
    )
    def test_it_should_get_destination_version_when_user_does_not_inform_a_specific_version(
        self, simplevirtuosomigrate_mock
    ):
    ...
    @patch(
        "neptune_migrate.main.SimpleVirtuosoMigrate",
        return_value=Mock(**{"check_if_version_exists.return_value": False}),
    )
    @patch(
        "neptune_migrate.main.Virtuoso",
        return_value=Mock(
    ...
    def test_it_should_raise_exception_when_get_destination_version_and_version_does_not_exist_on_migrations_dir(
        self, virtuoso_mock, simplevirtuosomigrate_mock
    ):
        self.initial_config.update({"schema_version": "20090214115900"})
        main = Main(Config(self.initial_config))
        self.assertRaisesWithMessage(
            Exception, "version not found (20090214115900)", main.execute
        )
        main.virtuoso_migrate.check_if_version_exists.assert_called_with(
            "20090214115900"
        )
    ...
    @patch("neptune_migrate.main.Main._execution_log")
    @patch("neptune_migrate.main.CLI.msg")
    def test_it_should_accept_file_migration_option_instead_a_schema_version(
        self, cli_mock, _execution_log_mock, virtuoso_mock, execute_migrations_mock
    ):
        self.initial_config.update(
            {"file_migration": "migration", "schema_version": None}
        )
        main = Main(Config(self.initial_config))
        main.execute()
        ...
        self.assertEqual(expected_calls, _execution_log_mock.mock_calls)
        execute_migrations_mock.assert_called_with(
            "sparql_up", "sparql_down", "current_version", "migration"
        )
    ...
    @patch("neptune_migrate.main.Main._execution_log")
    @patch("neptune_migrate.main.CLI.msg")
    def test_it_should_not_accept_file_migration_option_for_first_database_migration(
        self, cli_mock, _execution_log_mock, virtuoso_mock, execute_migrations_mock
    ):
        ...
        self.assertEqual(expected_calls, _execution_log_mock.mock_calls)
        cli_mock.assert_called_with(
            "[ERROR] Can't execute migration FROM None TO File (TIP: version it using git --tag and then use -m)\n",
            "RED",
        )
    ...
    @patch("neptune_migrate.main.Main._execute_migrations")
    @patch(
        "neptune_migrate.main.Virtuoso.get_current_version",
        return_value=("current_file", "file"),
    )
    @patch("neptune_migrate.main.Main._execution_log")
    @patch("neptune_migrate.main.CLI.msg")
    def test_it_should_not_accept_file_migration_option_if_current_version_is_already_a_file(
        ...
        self.assertEqual(expected_calls, _execution_log_mock.mock_calls)
        cli_mock.assert_called_with(
            "[ERROR] Can't execute migration FROM File TO File (TIP: version it using git --tag and then use -m)\n",
            "RED",
        )
        self.assertEqual(0, execute_migrations_mock.call_count)
    ...
    @patch(
        "neptune_migrate.main.SimpleVirtuosoMigrate.check_if_version_exists",
        return_value=True,
    )
    @patch("neptune_migrate.main.Main._execution_log")
    @patch("neptune_migrate.main.CLI.msg")
    def test_it_should_accept_a_schema_version_option_instead_file_migration(
        self,
        cli_mock,
        _execution_log_mock,
        simplevirtuosomigrate_mock,
        virtuoso_mock,
        execute_migrations_mock,
    ):
        self.initial_config.update(
            {"schema_version": "version", "file_migration": None}
        )
        main = Main(Config(self.initial_config))
        main.execute()
        ...
        self.assertEqual(expected_calls, _execution_log_mock.mock_calls)
        execute_migrations_mock.assert_called_with(
            "sparql_up", "sparql_down", "current_version", "version"
        )
    ...
    @patch(
        ...
        return_value=Mock(
            **{
                "get_current_version.return_value": (None, None),
                "get_sparql.return_value": ("sparql_up", "sparql_down"),
            }
        ),
    )
    ...
    @patch("neptune_migrate.main.Main._execution_log")
    @patch("neptune_migrate.main.CLI.msg")
    def test_it_should_accept_a_schema_version_option_for_first_database_migration(
        self,
        cli_mock,
        _execution_log_mock,
        simplevirtuosomigrate_mock,
        ...
        self.assertEqual(expected_calls, _execution_log_mock.mock_calls)
        execute_migrations_mock.assert_called_with(
            "sparql_up", "sparql_down", None, "version"
        )
    ...
    @patch(
        ...
        return_value=Mock(
            **{
                "get_current_version.return_value": ("current_file", "file"),
                "get_sparql.return_value": ("sparql_up", "sparql_down"),
            }
        ),
    )
    ...
    def test_it_should_accept_a_schema_version_option_if_current_version_is_a_file(
        ...
        self.assertEqual(expected_calls, _execution_log_mock.mock_calls)
        execute_migrations_mock.assert_called_with(
            "sparql_up", "sparql_down", "current_file", "version"
        )

    @patch("neptune_migrate.main.Virtuoso")
    @patch("neptune_migrate.main.Main._execution_log")
    def test_it_should_use_virtuoso_class_to_get_migration_and_execute_changes(
        ...
        self.assertEqual(expected_calls, _execution_log_mock.mock_calls)
        main.virtuoso.execute_change.assert_called_with(
            "sparql_up line 1\nsparql_up line 2\nsparql_up line 3",
            "sparql_down line 1\nsparql_down line 2\nsparql_down line 3",
    ...
    @patch("neptune_migrate.main.Virtuoso")
    @patch("neptune_migrate.main.Main._execution_log")
    def test_it_should_log_executed_sparql_if_asked_to_show_sparql(
        self, _execution_log_mock, virtuoso_mock
    ):
    ...
    @patch("neptune_migrate.main.Virtuoso")
    @patch("neptune_migrate.main.Main._execution_log")
    def test_it_should_not_execute_any_sparql_if_asked_to_show_sparql_only(
        self, _execution_log_mock, virtuoso_mock
    ):
