...
class CLITest(unittest.TestCase):
    ...
    def test_it_should_exit_with_help_options(self, stdout_mock):
        try:
            ...
            self.assertTrue(
                stdout_mock.getvalue().find(
                    "Displays simple-virtuoso-migrate's version and exit"
                )
                > 0
            )
    ...
    def test_it_should_accept_configuration_file_options(self):
        ...
        self.assertEqual(
            "file.conf", CLI.parse(["--config", "file.conf"])[0].config_file
        )
    ...
    def test_it_should_accept_log_level_options(self):
        self.assertEqual(
            "log_level_value", CLI.parse(["-l", "log_level_value"])[0].log_level
        )
        self.assertEqual(
            "log_level_value",
    ...
    def test_it_should_accept_log_dir_options(self):
        self.assertEqual(
            "log_dir_value", CLI.parse(["--log-dir", "log_dir_value"])[0].log_dir
        )
    ...
    def test_it_should_accept_schema_version_options(self):
        self.assertEqual(
            ...
            CLI.parse(["-g", "schema_version_value"])[0].schema_version,
        )
        self.assertEqual(
            "schema_version_value",
            CLI.parse(["--git", "schema_version_value"])[0].schema_version,
        )
    ...
    def test_it_should_accept_file_migration_options(self):
        self.assertEqual(
            "file_migration_value",
            CLI.parse(["-f", "file_migration_value"])[0].file_migration,
        )
        self.assertEqual(
            "file_migration_value",
            CLI.parse(["--file", "file_migration_value"])[0].file_migration,
        )
    ...
    def test_it_should_accept_load_ttl_options(self):
        self.assertEqual(
            "load_ttl_value", CLI.parse(["-a", "load_ttl_value"])[0].load_ttl
        )
        self.assertEqual(
            "load_ttl_value", CLI.parse(["--add", "load_ttl_value"])[0].load_ttl
        )
    ...
    def test_it_should_accept_simple_virtuoso_migrate_version_options(self):
        ...
        self.assertEqual(
            True, CLI.parse(["--version"])[0].simple_virtuoso_migrate_version
        )
    ...
    def test_it_should_accept_environment_options(self):
        self.assertEqual(
            "environment_value",
            CLI.parse(["--env", "environment_value"])[0].environment,
        )
        self.assertEqual(
            "environment_value",
            CLI.parse(["--environment", "environment_value"])[0].environment,
        )
    ...
    def test_it_should_accept_database_user_options(self):
        self.assertEqual(
            "user_value", CLI.parse(["--db-user", "user_value"])[0].database_user
        )
    ...
    def test_it_should_accept_database_password_options(self):
        self.assertEqual(
            "password_value",
            CLI.parse(["--db-password", "password_value"])[0].database_password,
        )
    ...
    def test_it_should_accept_database_host_options(self):
        self.assertEqual(
            "host_value", CLI.parse(["--db-host", "host_value"])[0].database_host
        )
    ...
    def test_it_should_accept_database_port_options(self):
        self.assertEqual(
            "port_value", CLI.parse(["--db-port", "port_value"])[0].database_port
        )
    ...
    def test_it_should_accept_database_endpoint_options(self):
        self.assertEqual(
            "endpoint_value",
            CLI.parse(["--db-endpoint", "endpoint_value"])[0].database_endpoint,
        )

    def test_it_should_accept_run_after_options(self):
        self.assertEqual(
            "script_name", CLI.parse(["--run-after", "script_name"])[0].run_after
        )
    ...
    def test_it_should_accept_database_graph_options(self):
        self.assertEqual(
            "graph_value", CLI.parse(["--db-graph", "graph_value"])[0].database_graph
        )
    ...
    def test_it_should_accept_database_ontology_options(self):
        self.assertEqual(
            "ontology_value",
            CLI.parse(["--db-ontology", "ontology_value"])[0].database_ontology,
        )
    ...
    def test_it_should_accept_migrations_dir_options(self):
        self.assertEqual(
            ".:../:/tmp",
            CLI.parse(["--db-migrations-dir", ".:../:/tmp"])[0].database_migrations_dir,
        )
