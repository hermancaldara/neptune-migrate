...
"""
    ...
    def test_it_should_raise_exception_config_file_not_exists(self):
        try:
            ...
            self.assertEqual(
                "%s: file not found" % os.path.abspath("unexistent.conf"), str(e)
            )
    ...
    @patch(
        "neptune_migrate.helpers.tempfile.NamedTemporaryFile",
        side_effect=IOError("some error"),
    )
    def test_it_should_raise_exception_when_an_error_happens_on_writing_temporary_file(
        ...
        try:
            ...
            self.assertEqual(
                "could not create temporary file for content_reference -> (some error)",
                str(e),
            )

    def test_normalize_non_negative_integer(self):
        literal = Literal(
            "1", datatype="http://www.w3.org/2001/XMLSchema#nonNegativeInteger"
        )
